...
class Aggregator(nn.Module):
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
            avg = corr.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] # shape = [B, T]
            classes = avg.topk(self.pad_len, dim=-1, sorted=False)[1] # shape = [B, pad_len]
            th_text = F.normalize(text_feats, dim=-1) # shape = [B, T, P, C]
            th_text = torch.gather(th_text, dim=1, index=classes[..., None, None].expand(-1, -1, th_text.size(-2), th_text.size(-1))) # shape = [B, pad_len, P, C]
            orig_classes = text_feats.size(1) # T
            img_feats = F.normalize(img_feats, dim=1) # shape = [B, C, H, W]
            text_feats = th_text # shape = [B, pad_len, P, C]
            corr = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text) # shape = [B, P, pad_len, H, W]
        # corr = self.feature_map(img_feats, text_feats)
        corr_embed = self.corr_embed(corr) # shape = [B, hidden_dim, T, H, W]
        ...
        logit = self.conv_decoder(corr_embed, projected_decoder_guidance) # shape = [B, T, H, W]
        if classes is not None:
            out = torch.full((logit.size(0), orig_classes, logit.size(2), logit.size(3)), -100., device=logit.device)
            out.scatter_(dim=1, index=classes[..., None, None].expand(-1, -1, logit.size(-2), logit.size(-1)), src=logit)
            logit = out
        return logit # shape = [B, class_num, image_height, image_width]
...
class CATSegDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        prompt_ensemble_type: str = None,
        text_guidance_dim: int = None,
        text_guidance_proj_dim: int = None,
        appearance_guidance_dim: int = None,
        appearance_guidance_proj_dim: int = None,
        prompt_depth: int = None,
        prompt_length: int = None,
        prompt_channel: int = None,
        decoder_dims: list = None,
        decoder_guidance_dims: list = None,
        decoder_guidance_proj_dims: list = None,
        num_heads: int = None,
        num_layers: tuple = None,
        hidden_dims: tuple = None,
        pooling_sizes: tuple = None,
        feature_resolution: tuple = None,
