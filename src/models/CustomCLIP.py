...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.text_features = nn.Parameter(
            self.build_ensemble_text_features(text_prompts, dtype=self.dtype))
    ...
    def forward(self, image, is_train=False, up=True, impaths=None):
        ...
        output['cls_token'] = res['pooled']

        origin_layers_image_feature = res['output_layers']
        image_features = origin_image_features / \
            origin_image_features.norm(dim=-1, keepdim=True)
        ...
        expand_text_features = text_features.repeat(
            image_features.shape[0], 1, 1)
        if up:
            # seg decoder
            patch_features = res['tokens']
            patch_features = self.seg_decoder(
                patch_features, expand_text_features, idx=-1)  # cross with text
            patch_features = patch_features / \
                patch_features.norm(dim=-1, keepdim=True)
