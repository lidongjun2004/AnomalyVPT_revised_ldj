...
class Aggregator(nn.Module):
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
            avg = corr.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] # (B, T)
            ...
            img_feats = F.normalize(img_feats, dim=1) # B C H W
            text_feats = th_text # B pad_len P C
            corr = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text) # (B, pad_len, T, H, W)
        #corr = self.feature_map(img_feats, text_feats)
        corr_embed = self.corr_embed(corr) # (B, hidden_dim, T, H, W)
